(function ($) {
  class Polygonizr {
    constructor($this, options) {
      ...
      this.setupClusterNodes = function () {
        ...
        for (
          let i = 0;
          i <
          this.settings.numberOfNodes + this.settings.numberOfUnconnectedNode;
          i++
        ) {
          ...
          currentNode.z =
            this.settings.node3dDepthDistance +
            this.settings.node3dDepthDistance * Math.cos(phi);

          // Set colors into arrays.
          this.settings.nodeDotColor = Array.isArray(this.settings.nodeDotColor)
            ? this.settings.nodeDotColor
            : new Array(this.settings.nodeDotColor);
          this.settings.nodeLineColor = Array.isArray(
            this.settings.nodeLineColor
          )
            ? this.settings.nodeLineColor
            : new Array(this.settings.nodeLineColor);
          this.settings.nodeFillColor = Array.isArray(
            this.settings.nodeFillColor
          )
            ? this.settings.nodeFillColor
            : new Array(this.settings.nodeFillColor);
          this.settings.nodeFillGradientColor = Array.isArray(
            this.settings.nodeFillGradientColor
          )
            ? this.settings.nodeFillGradientColor
            : new Array(this.settings.nodeFillGradientColor);

          // Set max if no overflow is allowed.
          if (this.settings.nodeOverflow == false) {
            // Calculate new max heights and widths.
            let maxDistance =
              this.settings.nodeMovementDistance + this.settings.nodeDotSize;
            ...
            this.m_nodeMovementDistance = Math.min(
              Math.min(this.settings.nodeMovementDistance, maxWidth),
              Math.min(this.settings.nodeMovementDistance, maxHeight)
            );

            // Alter the canvas position to force it inside the canvas.
            currentNode.x = Math.floor(
              currentNode.x + maxDistance > this.settings.canvasWidth
                ? maxWidth
                : currentNode.x
            );
            currentNode.x = Math.floor(
            ...
            currentNode.y = Math.floor(
              currentNode.y + maxDistance > this.settings.canvasHeight
                ? maxHeight
                : currentNode.y
            );
            currentNode.y = Math.floor(
          ...
          this.nodes[i].UnconnectedNode =
            this.settings.numberOfUnconnectedNode > i;
        }
        ...
        for (let i = 0; i < this.nodes.length; i++) {
          ...
          node.nodeDotColor =
            this.settings.nodeDotColor[
              this.settings.nodeDotColoringSchema ==
          ...
          node.nodeLineColor =
            this.settings.nodeLineColor[
              this.settings.nodeLineColoringSchema ==
              Constants.Coloring.COLORING_RANDOM
          ...
          node.nodeFillColor =
            this.settings.nodeFillColor[
              this.settings.nodeFillColoringSchema ==
              Constants.Coloring.COLORING_RANDOM
          ...
          node.nodeFillGradientColor =
            this.settings.nodeFillGradientColor[
              this.settings.nodeFillGradientColoringSchema ==
              Constants.Coloring.COLORING_RANDOM
                ? Math.floor(
                    Math.random() * this.settings.nodeFillGradientColor.length
                  )
      ...
      this.Animator = function (
        ...
        function step(timestamp) {
          ...
          let currentFrame = Math.floor(
            (timestamp - m_startTime) / (1000 / fps)
          );
        ...
        function setNewTargetPossition() {
          ...
          for (let i in $self.nodes) {
            ...
            let newTargetX = $self.calculateNewTargetPossition(
              $self.nodes[i].originX
            );
            let newTargetY = $self.calculateNewTargetPossition(
              $self.nodes[i].originY
            );
            ...
            $self.nodes[i].NodePrediction =
              $self.settings.nodeDotPrediction > 0 &&
              Math.random() <= $self.settings.nodeDotPrediction;
          ...
          if (
            $self.settings.node3dRotateAxis == Constants.Rotation.MEDIAN_AXIS
          ) {
            ...
            $self.settings.node3dRotateAxis == Constants.Rotation.LEFT_AXIS
          ) {
            m_rotationAxis = 0;
          } else if (
            $self.settings.node3dRotateAxis == Constants.Rotation.RIGHT_AXIS
          ) {
        ...
        function setNewNodePossition(easing, currentTime, endTime) {
          ...
          m_turnSpeed =
            (2 * Math.PI) /
            ($self.settings.duration * $self.settings.animationFps);
          m_turnAngle = (m_turnAngle + m_turnSpeed) % (2 * Math.PI);
          m_sinAngle = Math.sin(
            getEasing(
              $self.settings.node3dRotatEase,
              currentTime,
              m_turnSpeed,
              2 * Math.PI,
              endTime
          ...
          for (let i in $self.nodes) {
            // Normal movement.
            $self.nodes[i].currentX = getEasing(
              easing,
              currentTime,
              $self.nodes[i].startX,
              $self.nodes[i].targetX,
              endTime
            );
            $self.nodes[i].currentY = getEasing(
              easing,
              currentTime,
              $self.nodes[i].startY,
              $self.nodes[i].targetY,
              endTime
            );

            // Rotation logic.
            if (
              $self.settings.node3dRotate &&
              m_3dRotateOnNthNodeMovement %
                $self.settings.node3dRotateOnNthNodeMovement ==
                0
            ) {
              ...
              m_rotX =
                -m_cosAngle * m_dist +
                m_sinAngle *
                  ($self.nodes[i].originZ - $self.settings.node3dDepthDistance);
              m_rotZ =
                -m_sinAngle * m_dist +
                m_cosAngle *
                  ($self.nodes[i].originZ - $self.settings.node3dDepthDistance);
              ...
              if ($self.settings.nodeOverflow == false) {
                // Calculate new max heights and widths.
                let maxHeight =
                  $self.settings.canvasHeight - $self.settings.nodeDotSize;
                let maxWidth =
                  $self.settings.canvasWidth - $self.settings.nodeDotSize;

                // Alter the canvas position to force it inside the canvas.
                $self.nodes[i].currentX = Math.floor(
                  $self.nodes[i].currentX > maxWidth
                    ? maxWidth
                    : $self.nodes[i].currentX
                );
                $self.nodes[i].currentX = Math.floor(
                  $self.nodes[i].currentX < $self.settings.nodeDotSize
                    ? $self.settings.nodeDotSize
                    : $self.nodes[i].currentX
                );
                $self.nodes[i].currentY = Math.floor(
                  $self.nodes[i].currentY > $self.settings.canvasHeight
                    ? maxHeight
                    : $self.nodes[i].currentY
                );
                $self.nodes[i].currentY = Math.floor(
                  $self.nodes[i].currentY < $self.settings.nodeDotSize
                    ? $self.settings.nodeDotSize
                    : $self.nodes[i].currentY
                );
              }
              ...
              $self.nodes[i].zAlpha =
                $self.nodes[i].zAlpha > 1
                  ? 1
                  : $self.nodes[i].zAlpha < minAlpha
                  ? minAlpha
                  : $self.nodes[i].zAlpha;
        ...
        let m_requestAnimationFrame =
          window.requestAnimationFrame ||
          window.mozRequestAnimationFrame ||
          window.webkitRequestAnimationFrame ||
          window.oRequestAnimationFrame ||
          window.msRequestAnimationFrame;
        let m_cancleAnimationFrame =
      ...
      this.setAlphaLevel = function (node) {
        let screenDistance = Math.sqrt(
          Math.pow(this.settings.canvasWidth, 2) +
            Math.pow(this.settings.canvasHeight, 2)
        );
        let nodeDistance = 0;
      ...
      this.drawLines = function ($self, node) {
        ...
        for (let i in node.Closest) {
          ...
          let lineConnection =
            ...
            node.Closest[(i + 1) % node.Closest.length].UnconnectedNode ==
              false;
          let drawCloseUnconnection =
            $self.settings.ConnectUnconnectedNodes == true &&
            getDistance(node, node.Closest[i]) <=
              $self.settings.ConnectUnconnectedNodesDistance;
      ...
      this.drawFillNodeConnection = function ($self, node, i) {
        ...
        if (
          ...
          var gradient = $self.ctx.createLinearGradient(
            node.currentX,
            node.currentY,
    ...
    refresh() {
      ...
      this.animation = new this.Animator(
        this,
