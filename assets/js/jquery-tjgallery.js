...
(function ($) {
  ...
  function build(container) {
    ...
    container.each(function () {
      ...
      if (
        lastBucket.items.length === 1 &&
        buckets[buckets.length - 1].items.length > 1
      ) {
      ...
      $.each(buckets, function (_, bucket) {
        bucket.scale =
          (containerWidth - (bucket.items.length - 1) * settings.margin) /
          getWidthForBucket(bucket.items);

        $.each(bucket.items, function (index, item) {
          if (bucket.scale) {
...
// calling function on top container for gallery
